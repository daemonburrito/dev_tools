...
VENV_NAME=AppStore
VENV_BASE_PATH=envs
VENV_PATH=${BASE_PATH}/${VENV_BASE_PATH}/${VENV_NAME}
...
if [ ! -d $VENV_PATH ] ; then
	...
	pip -q install django-profiler==1.1
	deactivate
fi
...
rsync -ah --delete ../debian .
mkdir -p ${VENV_BASE_PATH}
rsync -ah --delete --exclude '*.pyc' ${VENV_PATH} ${VENV_BASE_PATH}/
rsync -ah --delete --exclude '*.pyc' ../${APPSTORE_CO_PATH} .
